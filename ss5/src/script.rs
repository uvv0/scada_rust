...
impl Script {
    ...
    pub fn eval_result(
        ...
        let mut step = || -> Result<(), String> {
            steps += 1;
            if steps > max_steps {
                return Err(format!("SCRIPT: step limit exceeded (maxSteps={})", max_steps));
            }
            Ok(())
        };
...
enum Stmt {
    ...
    Reg { reg_id_expr: Expr, value_expr: Expr },
    Emit { ts_expr: Expr, reg_id_expr: Expr, value_expr: Expr },
    Block(Vec<Stmt>),
    If { cond: Expr, then_branch: Box<Stmt>, else_branch: Option<Box<Stmt>> },
    While { cond: Expr, body: Box<Stmt> },
    For { init: Option<Box<Stmt>>, cond: Option<Expr>, step: Option<Box<Stmt>>, body: Box<Stmt> },
}
...
enum Expr {
    ...
    Call1 { func: Call1, a: Box<Expr> },
    Call2 { func: Call2, a: Box<Expr>, b: Box<Expr> },
    Call3 { func: Call3, a: Box<Expr>, b: Box<Expr>, c: Box<Expr> },
}
/// Function: $name.
fn parse_stmt(t: &mut Tokenizer, require_semicolon: bool) -> Result<Stmt, String> {
    ...
    if t.is_kw("while") {
        ...
        return Ok(Stmt::While { cond, body: Box::new(body) });
    }
    ...
    if t.is_kw("for") {
        ...
        let body = parse_stmt(t, true)?;
        return Ok(Stmt::For { init, cond, step, body: Box::new(body) });
    }
    ...
    if t.is_kw("if") {
        ...
        return Ok(Stmt::If { cond, then_branch: Box::new(then_branch), else_branch });
    }
    ...
    if t.is_kw("let") {
        ...
        return Ok(Stmt::Let { name: name_tok.text, expr });
    }
    ...
    if t.is_kw("reg") {
        ...
        return Ok(Stmt::Reg { reg_id_expr, value_expr });
    }
    ...
    if t.is_kw("emit") {
        ...
        return Ok(Stmt::Emit { ts_expr, reg_id_expr, value_expr });
    }

    if t.is_kw("print") && t.peek_ahead_text(1, "(") {
        return Err("SCRIPT: \"print(...)\" is not a statement. Use: let _ = print(...);".to_string());
    }
    ...
    if t.peek().kind == TokKind::Ident && is_ident(&t.peek().text) && t.peek_ahead_text(1, "=") {
        ...
        return Ok(Stmt::Assign { name: name_tok.text, expr });
    }

    Err(format!("SCRIPT: unknown statement near \"{}\"", t.peek().text))
}
...
fn parse_expr_until(t: &mut Tokenizer, stop_text: &str) -> Result<Expr, String> {
    ...
    loop {
        ...
        if tok.kind == TokKind::Eof {
            return Err(format!("SCRIPT: unexpected EOF while reading expression (expected \"{}\")", stop_text));
        }
...
/// Function: $name.
fn parse_expr_until_semicolon_or_stop(t: &mut Tokenizer, require_semicolon: bool) -> Result<Expr, String> {
    let start = t.peek().start;
    let mut depth = 0i32;
    let mut end = start;
    ...
    loop {
        ...
        if tok.kind == TokKind::Eof {
            if require_semicolon {
                return Err("SCRIPT: unexpected EOF while reading expression (expected \";\")".to_string());
            }
            break;
        }
        if depth == 0 && tok.text == ";" {
...
fn parse_for_header_stmt(t: &mut Tokenizer, part_name: &str) -> Result<Stmt, String> {
    if t.peek().kind == TokKind::Ident && t.peek().text == "let" {
        ...
        let name_tok = t.next();
        if name_tok.kind != TokKind::Ident || !is_ident(&name_tok.text) {
            return Err(format!("SCRIPT: bad var name \"{}\" in {}", name_tok.text, part_name));
        }
        t.expect("=")?;
...
impl<'a> ExprParser<'a> {
    ...
    fn parse_lor(&mut self) -> Result<Expr, String> {
        ...
        while self.t.peek_text("||") {
            self.t.next();
            e = Expr::Bin { op: BinOp::Or, l: Box::new(e), r: Box::new(self.parse_land()?) };
        }
        Ok(e)
    }
    ...
    fn parse_land(&mut self) -> Result<Expr, String> {
        ...
        while self.t.peek_text("&&") {
            self.t.next();
            e = Expr::Bin { op: BinOp::And, l: Box::new(e), r: Box::new(self.parse_cmp()?) };
        }
        Ok(e)
    }
    ...
    fn parse_cmp(&mut self) -> Result<Expr, String> {
        ...
        loop {
            if self.t.peek_text("<=") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Le, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else if self.t.peek_text("<") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Lt, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else if self.t.peek_text(">=") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Ge, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else if self.t.peek_text(">") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Gt, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else if self.t.peek_text("==") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Eq, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else if self.t.peek_text("!=") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Ne, l: Box::new(e), r: Box::new(self.parse_bor()?) };
            } else {
                break;
            }
        }
    ...
    fn parse_bor(&mut self) -> Result<Expr, String> {
        ...
        while self.t.peek_text("|") {
            self.t.next();
            e = Expr::Bin { op: BinOp::Bor, l: Box::new(e), r: Box::new(self.parse_bxor()?) };
        }
        Ok(e)
    }
    ...
    fn parse_bxor(&mut self) -> Result<Expr, String> {
        ...
        while self.t.peek_text("^") {
            self.t.next();
            e = Expr::Bin { op: BinOp::Bxor, l: Box::new(e), r: Box::new(self.parse_band()?) };
        }
        Ok(e)
    }
    ...
    fn parse_band(&mut self) -> Result<Expr, String> {
        ...
        while self.t.peek_text("&") {
            self.t.next();
            e = Expr::Bin { op: BinOp::Band, l: Box::new(e), r: Box::new(self.parse_shift()?) };
        }
        Ok(e)
    }
    ...
    fn parse_shift(&mut self) -> Result<Expr, String> {
        ...
        loop {
            if self.t.peek_text("<<") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Shl, l: Box::new(e), r: Box::new(self.parse_add()?) };
            } else if self.t.peek_text(">>") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Shr, l: Box::new(e), r: Box::new(self.parse_add()?) };
            } else {
                break;
            }
        }
    ...
    fn parse_add(&mut self) -> Result<Expr, String> {
        ...
        loop {
            if self.t.peek_text("+") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Add, l: Box::new(e), r: Box::new(self.parse_mul()?) };
            } else if self.t.peek_text("-") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Sub, l: Box::new(e), r: Box::new(self.parse_mul()?) };
            } else {
                break;
            }
        }
    ...
    fn parse_mul(&mut self) -> Result<Expr, String> {
        ...
        loop {
            if self.t.peek_text("*") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Mul, l: Box::new(e), r: Box::new(self.parse_unary()?) };
            } else if self.t.peek_text("/") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Div, l: Box::new(e), r: Box::new(self.parse_unary()?) };
            } else if self.t.peek_text("%") {
                self.t.next();
                e = Expr::Bin { op: BinOp::Mod, l: Box::new(e), r: Box::new(self.parse_unary()?) };
            } else {
                break;
            }
        }
    ...
    fn parse_unary(&mut self) -> Result<Expr, String> {
        if self.t.peek_text("-") {
            self.t.next();
            return Ok(Expr::Unary { op: UnaryOp::Neg, a: Box::new(self.parse_unary()?) });
        }
        self.parse_primary()
    }
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        ...
        if tok.kind == TokKind::Num {
            ...
            let v = if tok.text.contains('.') || tok.text.contains('e') || tok.text.contains('E') {
                tok.text.parse::<f64>().map_err(|e| format!("SCRIPT expr: bad number {}: {}", tok.text, e))?
            } else {
                tok.text.parse::<i64>().map_err(|e| format!("SCRIPT expr: bad int {}: {}", tok.text, e))? as f64
            };
            ...
            if self.t.peek_text("(") {
                ...
                if self.t.peek_text(",") {
                    ...
                    if self.t.peek_text(",") {
                        ...
                        self.t.expect(")")?;
                        return Ok(Expr::Call3 { func: map_call3(&name)?, a: Box::new(a), b: Box::new(b), c: Box::new(c) });
                    }
                    self.t.expect(")")?;
                    return Ok(Expr::Call2 { func: map_call2(&name)?, a: Box::new(a), b: Box::new(b) });
                }
                self.t.expect(")")?;
                return Ok(Expr::Call1 { func: map_call1(&name)?, a: Box::new(a) });
            }
        ...
        Err(format!("SCRIPT expr: bad token \"{}\" in \"{}\"", tok.text, self.s))
    }
}
...
fn compile_stmt(st: &Stmt, out: &mut Vec<Instr>) -> Result<(), String> {
    ...
    match st {
        ...
        Stmt::Reg { reg_id_expr, value_expr } => {
            compile_expr(reg_id_expr, out)?;
            compile_expr(value_expr, out)?;
            out.push(Instr::Reg);
        }
        Stmt::Emit { ts_expr, reg_id_expr, value_expr } => {
...
fn eval_bin(op: BinOp, a: f64, b: f64) -> f64 {
    match op {
        ...
        BinOp::Mod => {
            let bi = b as i64;
            if bi == 0 { 0.0 } else { (a as i64 % bi) as f64 }
        }
...
fn eval_call1(
    ...
    match fn1 {
        ...
        Call1::Abs => a.abs(),
        Call1::Sqrt => if a <= 0.0 { 0.0 } else { a.sqrt() },
        Call1::Floor => a.floor(),
        Call1::Ceil => a.ceil(),
        Call1::Round => a.round(),
    }
...
fn eval_call2(
    ...
    match fn2 {
        ...
        Call2::Min => if a < b { a } else { b },
        Call2::Max => if a > b { a } else { b },
        Call2::Pow => a.powf(b),
    }
}
...
fn dt2unix(raw_in: f64) -> f64 {
    ...
    let swap16_words = |v: i64| -> i64 {
        ...
        let hi = ((u >> 16) & 0xFFFF) as u32;
        let lo = (u & 0xFFFF) as u32;
        ((lo << 16) | hi) as i64
    };
...
impl<'a> Tokenizer<'a> {
    /// Function: $name.
    fn new(s: &'a str) -> Self {
        let mut t = Self { s, i: 0, next_tok: Tok { kind: TokKind::Eof, text: "".to_string(), start: 0, end: 0 } };
        let tok = t.read();
        t.next_tok = tok;
        t
    }
    ...
    fn expect(&mut self, t: &str) -> Result<(), String> {
        if self.next_tok.text != t {
            return Err(format!("SCRIPT: expected \"{}\", got \"{}\"", t, self.next_tok.text));
        }
        self.next();
        Ok(())
    }
    ...
    fn read(&mut self) -> Tok {
        ...
        if self.i >= self.s.len() {
            return Tok { kind: TokKind::Eof, text: "".to_string(), start: self.i, end: self.i };
        }
        ...
        if self.i + 1 < self.s.len() {
            ...
            if matches!(two, "<<" | ">>" | "<=" | ">=" | "==" | "!=" | "&&" | "||") {
                self.i += 2;
                return Tok { kind: TokKind::Sym, text: two.to_string(), start, end: self.i };
            }
        }
        ...
        if syms.contains(ch) {
            self.i += 1;
            return Tok { kind: TokKind::Sym, text: ch.to_string(), start, end: self.i };
        }

        if is_digit(ch) || ch == '.' {
            if ch == '.' && (self.i + 1 >= self.s.len() || !is_digit(self.s.as_bytes()[self.i + 1] as char)) {
                return Tok { kind: TokKind::Sym, text: ".".to_string(), start, end: self.i + 1 };
            }
        ...
        Tok { kind: TokKind::Sym, text: ch.to_string(), start, end: self.i + 1 }
    }
...
impl<'a> ExprTokenizer<'a> {
    ...
    fn expect(&mut self, t: &str) -> Result<(), String> {
        if self.next_tok.text != t {
            return Err(format!("SCRIPT expr: expected \"{}\", got \"{}\"", t, self.next_tok.text));
        }
        self.next();
        Ok(())
    }
    ...
    fn read(&mut self) -> Tok {
        ...
        if is_digit(ch) || ch == '.' {
            ...
            return Tok { kind: TokKind::Num, text: self.s[start..self.i].to_string(), start, end: self.i };
        }
        ...
        if is_ident_start(ch) {
            ...
            return Tok { kind: TokKind::Ident, text: self.s[start..self.i].to_string(), start, end: self.i };
        }
