...
#define UART_IS_RX        (0U)
   ...
   if (tx_port != rx_port)
   {
    ...
    /* Alternate-function, push-pull, pull-up, high speed: direct LL registers. */
    pin_pos = POSITION_VAL(tx_pin);
    MODIFY_REG(tx_port->MODER, 3UL << (pin_pos * 2U), 2UL << (pin_pos * 2U));
    CLEAR_BIT(tx_port->OTYPER, tx_pin);
    MODIFY_REG(tx_port->OSPEEDR, 3UL << (pin_pos * 2U), 2UL << (pin_pos * 2U));
    MODIFY_REG(tx_port->PUPDR, 3UL << (pin_pos * 2U), 1UL << (pin_pos * 2U));
    MODIFY_REG(tx_port->AFR[pin_pos >> 3U],
               0xFUL << ((pin_pos & 7U) * 4U),
               tx_af_num << ((pin_pos & 7U) * 4U));

    pin_pos = POSITION_VAL(rx_pin);
    MODIFY_REG(rx_port->MODER, 3UL << (pin_pos * 2U), 2UL << (pin_pos * 2U));
    CLEAR_BIT(rx_port->OTYPER, rx_pin);
    MODIFY_REG(rx_port->OSPEEDR, 3UL << (pin_pos * 2U), 2UL << (pin_pos * 2U));
    MODIFY_REG(rx_port->PUPDR, 3UL << (pin_pos * 2U), 1UL << (pin_pos * 2U));
    MODIFY_REG(rx_port->AFR[pin_pos >> 3U],
               0xFUL << ((pin_pos & 7U) * 4U),
               rx_af_num << ((pin_pos & 7U) * 4U));
...
static rt_err_t stm32_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    {
    case DATA_BITS_8:
        cr1 = LL_USART_DATAWIDTH_8B;
        break;
    case DATA_BITS_9:
        cr1 = LL_USART_DATAWIDTH_9B;
        break;
    default:
        cr1 = LL_USART_DATAWIDTH_8B;
        break;
    ...
    {
    case STOP_BITS_1:
        cr2 = LL_USART_STOPBITS_1;
        break;
    case STOP_BITS_2:
        cr2 = LL_USART_STOPBITS_2;
        break;
    default:
        cr2 = LL_USART_STOPBITS_1;
        break;
    ...
    MODIFY_REG(instance->CR1,
               ...
               USART_CR1_FIFOEN,
               cr1 | LL_USART_DIRECTION_TX_RX |
               LL_USART_OVERSAMPLING_16);
    MODIFY_REG(instance->CR2, USART_CR2_STOP, cr2);
    CLEAR_BIT(instance->CR3, USART_CR3_RTSE | USART_CR3_CTSE |
              USART_CR3_ONEBIT | USART_CR3_HDSEL |
              USART_CR3_IREN | USART_CR3_SCEN);
    instance->PRESC = LL_USART_PRESCALER_DIV1;
    LL_USART_SetBaudRate(instance, HAL_RCC_GetPCLK1Freq(),
                         LL_USART_PRESCALER_DIV1,
                         LL_USART_OVERSAMPLING_16, cfg->baud_rate);
