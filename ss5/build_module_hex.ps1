...
$moduleSlots = @(
    @{ Index = 0; Start = [Convert]::ToUInt32('90000000', 16) },
    @{ Index = 1; Start = [Convert]::ToUInt32('90001000', 16) },
    @{ Index = 2; Start = [Convert]::ToUInt32('90002000', 16) }
)
...
foreach ($slot in $moduleSlots) {
    $slotStart = [uint32]$slot.Start
    $slotEnd = [uint32]($slotStart + 4095)
    $binaryPath = Join-Path $OutputDir `
        ('module_slot{0}.bin' -f $slot.Index)
    $hexPath = Join-Path $OutputDir `
        ('module_slot{0}.hex' -f $slot.Index)

    Export-ModuleBinary -SlotStart $slotStart -SlotEnd $slotEnd `
        -BinaryPath $binaryPath
    Set-ModuleCrc -BinaryPath $binaryPath
    Export-ModuleHex -SlotStart $slotStart -BinaryPath $binaryPath `
        -HexPath $hexPath
    Write-Host "Module $($slot.Index) HEX: $hexPath"
}
