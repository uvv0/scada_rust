#!/usr/bin/env python3
"""
ss4 database initialization script.
Creates PostgreSQL database, user, and all required tables.

Usage:
    python init_db.py
    python init_db.py --db-name ss4_db --user ss4_user --password change-me
"""

import argparse
import os
import sys

try:
    import psycopg2
    from psycopg2.extensions import ISOLATION_LEVEL_AUTOCOMMIT
except ImportError:
    print("Installing psycopg2-binary...")
    os.system(f"{sys.executable} -m pip install psycopg2-binary")
    import psycopg2
    from psycopg2.extensions import ISOLATION_LEVEL_AUTOCOMMIT


def get_connection(dbname="postgres", user="postgres", password="postgres", host="localhost", port=5432):
    """Connect to PostgreSQL default database for admin operations."""
    return psycopg2.connect(
        dbname=dbname,
        user=user,
        password=password,
        host=host,
        port=port
    )


def create_database_and_user(conn, db_name, db_user, db_password):
    """Create database user and database if they don't exist."""
    conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
    cur = conn.cursor()

    # Check if user exists
    cur.execute("SELECT 1 FROM pg_roles WHERE rolname = %s", (db_user,))
    if not cur.fetchone():
        cur.execute(f"CREATE USER {db_user} WITH PASSWORD %s", (db_password,))
        print(f"Created user: {db_user}")
    else:
        print(f"User {db_user} already exists")

    # Check if database exists
    cur.execute("SELECT 1 FROM pg_database WHERE datname = %s", (db_name,))
    if not cur.fetchone():
        cur.execute(f"CREATE DATABASE {db_name} OWNER {db_user}")
        print(f"Created database: {db_name}")
    else:
        print(f"Database {db_name} already exists")

    cur.close()
    conn.set_isolation_level(ISOLATION_LEVEL_READ_COMMITTED)


def run_schema(conn, schema_sql, description):
    """Run a schema SQL string."""
    print(f"Applying: {description}")
    cur = conn.cursor()
    cur.execute(schema_sql)
    conn.commit()
    cur.close()
    print(f"  Done.")


def main():
    parser = argparse.ArgumentParser(description="Initialize ss4 PostgreSQL database")
    parser.add_argument("--db-name", default=os.environ.get("PG_DB", "ss4_db"))
    parser.add_argument("--db-user", default=os.environ.get("PG_USER", "ss4_user"))
    parser.add_argument("--db-password", default=os.environ.get("PG_PASS", "change-me"))
    parser.add_argument("--pg-host", default=os.environ.get("PG_HOST", "localhost"))
    parser.add_argument("--pg-port", default=int(os.environ.get("PG_PORT", "5432")))
    parser.add_argument("--pg-admin-user", default="postgres")
    parser.add_argument("--pg-admin-password", default="postgres")
    args = parser.parse_args()

    print(f"Connecting to PostgreSQL at {args.pg_host}:{args.pg_port}...")

    # Connect as admin to create database and user
    try:
        admin_conn = get_connection(
            dbname="postgres",
            user=args.pg_admin_user,
            password=args.pg_admin_password,
            host=args.pg_host,
            port=args.pg_port
        )
    except Exception as e:
        print(f"Failed to connect to PostgreSQL: {e}")
        print("Make sure PostgreSQL is running and credentials are correct.")
        sys.exit(1)

    create_database_and_user(admin_conn, args.db_name, args.db_user, args.db_password)
    admin_conn.close()

    # Connect to the new database
    print(f"\nConnecting to {args.db_name}...")
    db_conn = get_connection(
        dbname=args.db_name,
        user=args.db_user,
        password=args.db_password,
        host=args.pg_host,
        port=args.pg_port
    )

    # Base tables
    print("\n=== Creating base tables ===")

    # Lookup tables: ip, port, n_mb
    run_schema(db_conn, """
        create table if not exists ip (
            id serial primary key,
            name text not null unique
        );

        create table if not exists port (
            id serial primary key,
            name text not null unique
        );

        create table if not exists n_mb (
            id serial primary key,
            name text not null unique
        );
    """, "Lookup tables (ip, port, n_mb)")

    # obj table
    run_schema(db_conn, """
        create table if not exists obj (
            id serial primary key,
            name text,
            ip text,
            port text,
            kanal integer,
            speed integer,
            stop integer,
            parit integer,
            bit integer
        );
    """, "obj table")

    # kpz table
    run_schema(db_conn, """
        create table if not exists kpz (
            id serial primary key,
            name text,
            rtu integer not null,
            obj integer not null references obj(id),
            modem integer not null,
            grups bytea,
            max_pkt_len integer not null default 256,
            start integer not null default 0,
            t_a integer not null default 6000,
            t_script integer not null default 6000,
            en_post boolean not null default false,
            updated_at timestamptz not null default now()
        );

        create index if not exists idx_kpz_obj on kpz(obj);
    """, "kpz table")

    # reg table
    run_schema(db_conn, """
        create table if not exists reg (
            id serial primary key,
            name text not null default '',
            addr integer not null,
            n_mb integer references n_mb(id),
            tip integer not null default 0,
            bits integer,
            grup integer,
            a_en boolean not null default false,
            a_no_write integer not null default 0,
            updated_at timestamptz not null default now()
        );

        create index if not exists idx_reg_addr on reg(addr);
        create index if not exists idx_reg_grup on reg(grup);
    """, "reg table")

    # g_script table
    run_schema(db_conn, """
        create table if not exists g_script (
            grup integer primary key,
            pre_src text,
            post_src text,
            max_k integer,
            max_words integer,
            en boolean,
            ver integer,
            updated_at timestamptz not null default now()
        );
    """, "g_script table")

    # script_binding table
    run_schema(db_conn, """
        create table if not exists script_binding (
            id bigserial primary key,
            kpz_id integer not null references kpz(id) on delete cascade,
            grup integer not null,
            logical integer not null,
            reg_id integer null references reg(id) on delete set null,
            addr integer null,
            enabled boolean not null default true,
            updated_at timestamptz not null default now(),
            unique (kpz_id, grup, logical)
        );

        create index if not exists ix_script_binding_kpz_grup
            on script_binding(kpz_id, grup);

        create index if not exists ix_script_binding_reg_id
            on script_binding(reg_id);
    """, "script_binding table")

    # arx_val table (archive values)
    run_schema(db_conn, """
        create table if not exists arx_val (
            kpz_id integer not null,
            reg_id integer not null,
            ts_unix bigint not null,
            tip integer not null,
            val_num double precision,
            val_raw bytea,
            primary key (kpz_id, reg_id, ts_unix)
        );

        create index if not exists idx_arx_val_ts on arx_val(ts_unix desc);
        create index if not exists idx_arx_val_kpz_reg on arx_val(kpz_id, reg_id);
    """, "arx_val table")

    # arx_state table
    run_schema(db_conn, """
        create table if not exists arx_state (
            kpz_id integer not null,
            arx_id integer not null,
            last_ind integer not null default 0,
            updated_at timestamptz not null default now(),
            primary key (kpz_id, arx_id)
        );
    """, "arx_state table")

    # poll_log table
    run_schema(db_conn, """
        create table if not exists poll_log (
            id bigserial primary key,
            ts timestamptz not null default now(),
            kpz_id integer not null references kpz(id) on delete cascade,
            kind text not null,
            msg text
        );

        create index if not exists idx_poll_log_ts on poll_log(ts desc);
        create index if not exists idx_poll_log_kpz on poll_log(kpz_id);
    """, "poll_log table")

    # elam table (for delta writes)
    run_schema(db_conn, """
        create table if not exists elam (
            ctid tid not null,
            kpz_id integer not null,
            reg_id integer not null,
            val_num double precision,
            val_raw bytea,
            ts timestamptz not null default now()
        );

        create index if not exists idx_elam_kpz_reg on elam(kpz_id, reg_id);
    """, "elam table")

    # scheduler_runtime_cfg
    run_schema(db_conn, """
        create table if not exists scheduler_runtime_cfg (
            id bigserial primary key,
            no_response_failures integer not null default 3,
            no_response_backoff_sec bigint not null default 600,
            metrics_p95_warn_ms bigint not null default 1000,
            metrics_p95_crit_ms bigint not null default 3000,
            modbus_a_timeout_ms bigint not null default 1800,
            modbus_script_timeout_ms bigint not null default 2600,
            updated_at timestamptz not null default now()
        );

        insert into scheduler_runtime_cfg
            (id, no_response_failures, no_response_backoff_sec,
             metrics_p95_warn_ms, metrics_p95_crit_ms,
             modbus_a_timeout_ms, modbus_script_timeout_ms)
        values (1, 3, 600, 1000, 3000, 1800, 2600)
        on conflict (id) do nothing;
    """, "scheduler_runtime_cfg table")

    # Alarm schema
    print("\n=== Applying alarm schema ===")
    alarm_sql = open("alarm_schema.sql").read() if os.path.exists("alarm_schema.sql") else ""
    if alarm_sql:
        run_schema(db_conn, alarm_sql, "alarm_schema.sql")

    # UI schema
    print("\n=== Applying UI schema ===")
    ui_sql = open("ui_schema.sql").read() if os.path.exists("ui_schema.sql") else ""
    if ui_sql:
        run_schema(db_conn, ui_sql, "ui_schema.sql")

    # Seed data
    print("\n=== Inserting seed data ===")
    cur = db_conn.cursor()

    # Default lookup values
    cur.execute("INSERT INTO ip (name) VALUES ('127.0.0.1') ON CONFLICT (name) DO NOTHING")
    cur.execute("INSERT INTO port (name) VALUES ('502') ON CONFLICT (name) DO NOTHING")
    cur.execute("INSERT INTO n_mb (name) VALUES ('default') ON CONFLICT (name) DO NOTHING")

    db_conn.commit()
    cur.close()
    db_conn.close()

    print("\n=== Database initialization complete ===")
    print(f"Database: {args.db_name}")
    print(f"User:     {args.db_user}")
    print(f"Password: {args.db_password}")
    print(f"Host:     {args.pg_host}:{args.pg_port}")
    print("\nUpdate .env file with these credentials.")


if __name__ == "__main__":
    main()
