...
mod tests {
    ...
    #[test]
    fn sout_mb_only_read_fc3_builds_valid_frame() {
        let frame = sout_mb_only(1, 3, 0x1234, 2, None).expect("fc3 frame should be built");
        assert_eq!(&frame[..6], &[1, 3, 0x12, 0x34, 0x00, 0x02]);
        let got_crc = ((frame[frame.len() - 1] as u16) << 8) | (frame[frame.len() - 2] as u16);
        assert_eq!(crc16(&frame[..frame.len() - 2]), got_crc);
    }

    #[test]
    fn sout_mb_only_rejects_invalid_write_func() {
        let err = sout_mb_only(1, 3, 0, 1, Some(&[0x00, 0x00]))
            .expect_err("invalid write tip must fail");
        assert!(err.contains("unsupported write func"));
    }

    #[test]
    fn extract_modbus_frame_skips_10_byte_header() {
        let mut pkt = vec![0u8; 10];
        pkt.extend_from_slice(&[1, 3, 0, 0]);
        assert_eq!(extract_modbus_frame(&pkt), Some(&pkt[10..]));
        assert!(extract_modbus_frame(&[0u8; 12]).is_none());
    }

    #[test]
    fn build_mb_chunks_splits_on_limit() {
        let frames = vec![vec![1u8; 10], vec![2u8; 10], vec![3u8; 10]];
        let chunks = build_mb_chunks(&frames, 45).expect("chunking should succeed");
