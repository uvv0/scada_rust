...
- The release binary path is always `target/release/ss6.exe`.
- Do not switch to an alternate target directory unless the user explicitly asks for it.
- If `target/release/ss6.exe` is locked and cannot be replaced, report that fact and wait for user direction.

## Communication

- Treat rebuilding `target/release/ss6.exe` as part of the normal workflow after changes.
